...
$categories = $wpdb->get_results("
    ...
    WHERE B.parent = '0'
    AND B.taxonomy = 'housing_category'
    ORDER BY B.parent, A.term_order ASC
");
	//取得したカテゴリへの各種処理
	foreach($categories as $val){
		//カテゴリのリンクURLを取得
		$cat_link = get_term_link($val->slug,'housing_category');
...
$category_children = $wpdb->get_results("
    ...
    FROM $wpdb->terms AS A
	LEFT OUTER JOIN $wpdb->term_taxonomy AS B ON A.term_id = B.term_id
    WHERE B.taxonomy = 'housing_category'
...
");
			...
			foreach($category_children as $child_val){
				$cat_link = get_term_link($child_val -> slug,'housing_category');
...
$category_children = $wpdb->get_results("
    ...
    FROM $wpdb->terms AS A
	LEFT OUTER JOIN $wpdb->term_taxonomy AS B ON A.term_id = B.term_id
    WHERE B.taxonomy = 'housing_tag'
	AND B.count != '0'
    ORDER BY A.term_order ASC
");
		//カテゴリのリスト出力
		foreach($category_children as $child_val){
			$cat_link = get_term_link($child_val -> slug,'housing_tag');
